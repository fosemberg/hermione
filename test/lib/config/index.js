...
describe('config', () => {
    ...
    const initConfig = (opts) => {
        ...
        const configOrConfigPath = opts.configOrConfigPath || defaults.config;
        const stubs = {
            './options': parseOptions
        };

        if (typeof configOrConfigPath === 'string') {
            const resolvedConfigPath = path.resolve(process.cwd(), configOrConfigPath);
            stubs[resolvedConfigPath] = opts.requireConfigReturns || {};
        }
        const Config = proxyquire('../../../lib/config', stubs);

        return Config.create(opts.configOrConfigPath, opts.allowOverrides);
    };
    ...
    describe('constructor', () => {
        ...
        it('should parse config from object', () => {
            initConfig({configOrConfigPath: {someOption: 'some-value'}});
        ...
        it('should extend config with a config path', () => {
            assert.include(initConfig({configOrConfigPath: 'config-path'}), {configPath: 'config-path'});
        });
